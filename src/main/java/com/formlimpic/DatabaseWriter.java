    ...
    private void save(ReceiptStore.Event e) {
        ...
        switch (p.getProperty("type")) {
            ...
            case "form" -> {
                ...
                jdbc.update("INSERT INTO forms (id, owner_id, title, content, starts_at, expires_at, has_bubble) VALUES (?::uuid, ?, ?, ?, ?, ?, ?) ON CONFLICT (id) DO UPDATE SET has_bubble = EXCLUDED.has_bubble", p.getProperty("id"), ownerId, p.getProperty("title"), p.getProperty("content"), time(p, "start"), time(p, "end"), hasBubble);
            }
